...
Get the SHA with `git log --oneline`
a808698     Fourth commit  
ca0bbb4     Third commit
5ffcac5     Second commit
ac49968     First commit
Move to previous commit and put Forth commit files back to staging area `git reset --soft ca0bbb4`
Edit these files  
Commit the files with `git commit -m 'some message'`
Push them back up with `git push -f` (-f because you are now behind the remote counterpart, and you will receive a terminal warning saying so)
